        ...
        if (is_array($plan_title_array)){        
            foreach ($plan_title_array as $key=> $plan_name) {
                ...
                <tr >
                    ...
                    <td class="">'.$plan_size.'</td>
                    <td class="">'.$plan_rooms.'</td>    
                    <td class="">';
                        if($plan_price!=''){
                            print  __('price: ','wpestate').' '.wpestate_show_price_floor($plan_price,$currency,$where_currency,1);
                        }
                        print'</td> 
                    <td class="text-align:center;"> Available </td>
                    <td class="text-align:center;"><a href="'.$full_img[0].'" rel="prettyPhoto[pp_floor]" data-toggle="lightbox"><input style="width:100%" type="submit" class="wpb_button  wpb_btn-info wpb_btn-large" id="floor_submit" value="Floor Plan"></a></td>
                </tr>';
                $show='';
            }
        }
...
//    $property_state     = esc_html(get_post_meta($post_id, 'property_state', true) );
 ...
 //   $property_state     = esc_html(get_post_meta($post_id, 'property_state', true) );
    ...
    $return_string1='<div class="row">';
    
    if ($property_address != ''){
        $return_string1.='<div class="listing_detail col-md-4"><strong>'.__('Address','wpestate').':</strong> ' . $property_address . '</div>'; 
    }
    if ($property_city != ''){
        $return_string1.= '<div class="listing_detail col-md-4"><strong>'.__('City','wpestate').':</strong> ' .$property_city. '</div>';  
    }  
    if ($property_area != ''){
        $return_string1.= '<div class="listing_detail col-md-4"><strong>'.__('Area','wpestate').':</strong> ' .$property_area. '</div>';
    }    
    if ($property_county != ''){
        $return_string1.= '<div class="listing_detail col-md-4"><strong>'.__('State/County','wpestate').':</strong> ' . $property_county . '</div>'; 
    }
    ...
    if ($property_zip != ''){
        $return_string1.= '<div class="listing_detail col-md-4"><strong>'.__('Zip','wpestate').':</strong> ' . $property_zip . '</div>';
    }  
    
    
    if ($property_country != '') {
        $return_string1.= '<div class="listing_detail col-md-4"><strong>'.__('Country','wpestate').':</strong> ' . $property_country . '</div>'; 
    } 
    ...
    $return_string2='<div class="row">';
    $return_string2.='<div class="listing_detail col-md-4" id="propertyid_display"><strong>'.__('Property Id ','wpestate'). ':</strong> '.$post_id.'</div>';
    if ($price !='' ){ 
        $return_string2.='<div class="listing_detail col-md-4"><strong>'.__('Price','wpestate'). ':</strong> '. $price.'</div>';
    }
  //  print '<div class="listing_detail col-md-4"><strong>'.__('Listing Id: ','wpestate').' LST-'.$post_id.'</div>';
    
    if ($property_size2 != ''){
        $return_string2.= '<div class="listing_detail col-md-4"><strong>'.__('Property Size','wpestate').':</strong> ' . $property_size . '</div>';
    }               
    if ($property_lot_size != ''){
        $return_string2.= '<div class="listing_detail col-md-4"><strong>'.__('Property Lot Size','wpestate').':</strong> ' . $property_lot_size . '</div>';
    }      
    if ($property_rooms2 != ''){
        $return_string2.= '<div class="listing_detail col-md-4"><strong>'.__('Rooms','wpestate').':</strong> ' . $property_rooms . '</div>'; 
    }      
    if ($property_bedrooms != ''){
        $return_string2.= '<div class="listing_detail col-md-4"><strong>'.__('Bedrooms','wpestate').':</strong> ' . $property_bedrooms . '</div>'; 
    }     
    if ($property_bathrooms != '')    {
        $return_string2.= '<div class="listing_detail col-md-4"><strong>'.__('Bathrooms','wpestate').':</strong> ' . $property_bathrooms . '</div>'; 
    }      
    ...
    if( !empty($custom_fields)){  
       ...
       //    $slug =   sanitize_key ( str_replace(' ','_',$name) );
           ...
           if($value!=''){
               $return_string2.= '<div class="listing_detail col-md-4"><strong>'.ucwords($label).':</strong> ' .$value. '</div>'; 
           }
           $i++;       
        }
    }
     ...
     $show_no_features= esc_html ( get_option('wp_estate_show_no_features','') );
        ...
        if($show_no_features!='no'){
            foreach($feature_list_array as $checker => $value){
                    ...
                    if (esc_html( get_post_meta($post_id, $input_name, true) ) == 1) {
                         $return_string3 .= '<div class="listing_detail col-md-4"><i class="fa fa-check"></i>' . trim($value) . '</div>';
                    }else{
                        $return_string3  .=  '<div class="listing_detail col-md-4"><i class="fa fa-times"></i>' . trim($value). '</div>';
                    }
              }
        }else{
    ...
    return $return_string1.='</div>'.$return_string2.'</div>'.$return_string3.'</div>';
}
